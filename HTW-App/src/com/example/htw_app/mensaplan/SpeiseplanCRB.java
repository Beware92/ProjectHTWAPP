...
public class SpeiseplanCRB extends Activity {
	private ProgressDialog pDialog;
	public static SpeiseplanCRB ma;
	...
	private static final String URL_ALL_DATA_CRB = "http://htwapp.ohost.de/get_all_data_crb.php";
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.action_mensa_refresh:
			if (isOnline()) {
				...
				Toast.makeText(SpeiseplanCRB.this,
	...
	public void doRawSelectMenue() {
		...
		String sql = "SELECT * FROM " + DatabaseHelper.TABLE_MENUE_CRB
				+ " ORDER BY " + DatabaseHelper.MENUE_CRB_FIELD_ID;
		Cursor result = db.rawQuery(sql, null);

		if (result.moveToFirst()) {
			int idIdx = result.getColumnIndex(DatabaseHelper.MENUE_CRB_FIELD_ID);
			int tagIdx = result.getColumnIndex(DatabaseHelper.MENUE_CRB_FIELD_TAG);
			int menue1Idx = result
					.getColumnIndex(DatabaseHelper.MENUE_CRB_FIELD_MENUE1);
			int menue2Idx = result
					.getColumnIndex(DatabaseHelper.MENUE_CRB_FIELD_MENUE2);
			do {
	...
	class LoadAllProducts extends AsyncTask<String, String, String> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			pDialog = new ProgressDialog(SpeiseplanCRB.this);
		...
		protected String doInBackground(String... args) {
			...
			try {
				...
				JSONObject json = jParser.makeHttpRequest(URL_ALL_DATA_CRB, "GET",
						params);
				...
				if (success == 1) {
					...
					for (int i = 0; i < products.length(); i++) {
						...
						values.put(DatabaseHelper.MENUE_CRB_FIELD_TAG, name);
						values.put(DatabaseHelper.MENUE_CRB_FIELD_MENUE1, menue1);
						values.put(DatabaseHelper.MENUE_CRB_FIELD_MENUE2, menue2);

						db.update(DatabaseHelper.TABLE_MENUE_CRB, values,
								DatabaseHelper.MENUE_CRB_FIELD_ID + "= '" + id
										+ "'", null);
						db.close();
					}
				}
